      ...
      <main className="mx-auto max-w-6xl px-4 pb-20 sm:px-6">
        ...
        <GuideSection number="06" title="Charts" intro="Diagrammet ska besvara en fråga. Axlar, baslinjer och jämförelser måste synas innan animationen börjar." >
          <div id="charts" className="grid border-2 border-foreground lg:grid-cols-[1.3fr_.7fr]">
            <div className="p-6 lg:border-r-2 lg:border-foreground"><p className="font-mono text-[10px] font-bold uppercase text-muted-foreground">Exempel / lager under kampanj</p><div className="mt-8 flex h-52 items-end gap-4 border-b-2 border-foreground">{[92,72,36,3,3,3,55].map((height, index) => <div key={index} className="flex flex-1 flex-col justify-end"><span className={`mb-2 text-center font-mono text-[10px] font-bold ${index > 2 && index < 6 ? "text-loss" : ""}`}>{index > 2 && index < 6 ? "0" : height}</span><div className={`chart-rise w-full border-x border-t border-foreground ${index > 2 && index < 6 ? "bg-loss" : "bg-foreground"}`} style={{ height: `${height}%`, animationDelay: `${index * 70}ms` }} /></div>)}</div><div className="mt-3 grid grid-cols-7 text-center font-mono text-[9px] font-bold"><span>MÅN</span><span>TIS</span><span>ONS</span><span>TOR</span><span>FRE</span><span>LÖR</span><span>SÖN</span></div></div>
